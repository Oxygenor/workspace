...
`sourceUrl` — без описів, коментарів, вкладень) з інстансу Kanboard і
синхронізує їх у Supabase-базу основного застосунку. Деплоїться **окремо**
від основного Vite-застосунку — Supabase Edge Functions працюють на Deno і
не можуть запускати справжній браузер, тож ця частина потребує власного
постійно запущеного Node-хоста.

Синхронізуються лише картки, **призначені на обліковий запис**, під яким
логіниться воркер (фільтр за `assignees`, `src/scrape.js`) — чужі картки на
спільній дошці не створюються взагалі. Колонка кожної синхронізованої
картки **віддзеркалює її поточний список у Qplaze** на кожному запуску
(мапінг `qplaze_column_map` у Postgres) — якщо хтось перетягнув картку на
Qplaze в іншу колонку, локальна картка теж переїде, і якщо є Telegram-бот
підключений (`user_integrations.telegram_chat_id`), прийде сповіщення про
це переміщення. Списки Qplaze без запису в `qplaze_column_map` (напр. його
власний "Архів") просто ніколи не синхронізуються.
...
## Telegram-сповіщення про переміщення

Необов'язково: якщо задати `TELEGRAM_BOT_TOKEN` (те саме значення, що й
секрет `TELEGRAM_BOT_TOKEN` у Supabase Edge Functions) — при кожному
переміщенні синхронізованої картки в іншу колонку всім учасникам workspace
з підключеним Telegram (`user_integrations.telegram_chat_id`) прийде
повідомлення на кшталт "🔄 Вашу картку «...» перенесено з «...» у «...»".
Без цієї змінної синхронізація працює так само, просто без сповіщень.
